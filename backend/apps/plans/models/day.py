...
class Day(Nutrients):
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.breakfast_flag = self.breakfast_exc or (
            self.intakes.filter(meal=Intake.MEAL_BREAKFAST)
            .filter(processed=True)
            .exists()
        )
        self.lunch_flag = self.lunch_exc or (
            self.intakes.filter(meal=Intake.MEAL_LUNCH)
            .filter(processed=True)
            .exists()
        )
        self.snack_flag = self.snack_exc or (
            self.intakes.filter(meal=Intake.MEAL_SNACK)
            .filter(processed=True)
            .exists()
        )
        self.dinner_flag = self.dinner_exc or (
            self.intakes.filter(meal=Intake.MEAL_DINNER)
            .filter(processed=True)
            .exists()
        )
        self.exercises_flag = self.exercises_exc or self.exercises.exists()
